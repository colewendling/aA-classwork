  ...
  WHERE
      movies.id IN (
        ...
        FROM
          movies
        JOIN
          castings ON castings.movie_id = movies.id --subquery of all moves with julie andrews in them
        JOIN
          actors ON actors.id = castings.actor_id
        WHERE
          actors.name = 'Julie Andrews'
      ) AND castings.ord = 1 --actor is the lead role
    ORDER BY
      movies.title; --sort alpha
  SQL
end
